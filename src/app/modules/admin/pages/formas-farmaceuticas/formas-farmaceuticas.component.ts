...
import { pharmaceuticalFormQueryKeys } from '../../../../core/query/pharmaceutical-form-query.keys';
...
@Component({
  selector: 'app-formas-farmaceuticas',
  standalone: true,
  imports: [
    QueryPageStatePipe,
    CommonModule,
  ...
  templateUrl: './formas-farmaceuticas.component.html',
})
export class FormasFarmaceuticasComponent {
  ...
  protected readonly listQuery = injectQuery(() => ({
    queryKey: pharmaceuticalFormQueryKeys.list({
    ...
    queryFn: () =>
      firstValueFrom(
        this.api.listPharmaceuticalFormsPaged({
  ...
  protected readonly editing = signal<PharmaceuticalFormItemDto | null>(null);
  protected readonly nombre = signal('');

  protected readonly deleteConfirmOpen = signal(false);
  protected readonly deleting = signal<PharmaceuticalFormItemDto | null>(null);

  protected readonly createMutation = injectMutation(() => ({
    mutationFn: (body: CreatePharmaceuticalFormRequest) =>
      firstValueFrom(this.api.createPharmaceuticalForm(body)),
    onSuccess: () => {
      this.notify.success('Forma farmacéutica creada correctamente');
      this.closeFormModal(true);
      void this.queryClient.invalidateQueries({ queryKey: pharmaceuticalFormQueryKeys.all });
    },
    onError: (err) => {
      this.notify.error(httpErrorMessage(err, 'No se pudo crear la forma farmacéutica'));
    },
  }));

  protected readonly updateMutation = injectMutation(() => ({
    mutationFn: ({ id, body }: { id: string; body: CreatePharmaceuticalFormRequest }) =>
      firstValueFrom(this.api.updatePharmaceuticalForm(id, body)),
    onSuccess: () => {
      this.notify.success('Forma farmacéutica actualizada correctamente');
      this.closeFormModal(true);
      void this.queryClient.invalidateQueries({ queryKey: pharmaceuticalFormQueryKeys.all });
    },
    onError: (err) => {
      this.notify.error(httpErrorMessage(err, 'No se pudo actualizar la forma farmacéutica'));
    },
  }));

  protected readonly deleteMutation = injectMutation(() => ({
    mutationFn: (id: string) => firstValueFrom(this.api.deletePharmaceuticalForm(id)),
    onSuccess: () => {
      this.notify.success('Forma farmacéutica eliminada correctamente');
      this.closeDeleteConfirm();
      void this.queryClient.invalidateQueries({ queryKey: pharmaceuticalFormQueryKeys.all });
    },
    onError: (err) => {
      this.notify.error(httpErrorMessage(err, 'No se pudo eliminar la forma farmacéutica'));
    },
  }));
  ...
  protected openEditModal(row: PharmaceuticalFormItemDto) {
  ...
  protected submitForm() {
    ...
    const body: CreatePharmaceuticalFormRequest = { nombre };
  ...
  protected openDeleteConfirm(row: PharmaceuticalFormItemDto) {
