...
@Component({
  ...
  template: `
    ...
    <app-page-toolbar>
      <h1 data-toolbar-title class="text-title-sm font-semibold">Balance y margen</h1>
    </app-page-toolbar>
    ...
    <app-page-state [loading]="marginQuery.isPending()" [error]="marginError()" (retry)="marginQuery.refetch()">
      @if (marginQuery.data(); as m) {
        ...
        <app-component-card title="Top productos por margen" className="mt-4">
          <table class="min-w-full text-sm">
            <thead><tr class="border-b text-gray-500"><th class="py-2">Producto</th><th class="py-2">Margen</th></tr></thead>
            <tbody>
              @for (p of m.topProducts; track p.productId) {
                <tr class="border-b border-gray-100"><td class="py-2">{{ p.nombre }}</td><td class="py-2">{{ p.margen | currency: 'PEN' }}</td></tr>
              }
            </tbody>
          </table>
        </app-component-card>
...
export class BalanceComponent {
  ...
  protected marginError() {
    ...
    return err ? httpErrorMessage(err, 'No se pudo cargar el margen') : null;
  }
}
